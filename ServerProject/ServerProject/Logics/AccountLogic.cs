...
namespace ServerProject.Logics
{
	...
	/// 賬戶邏輯類別，實現帳戶相關的業務邏輯
	/// </summary>
	public class AccountLogic : LogicBase, IAccountLogic
    {
        /// <summary>
        ...
        /// <param name="accountService">帳戶服務實例</param>
        ...
        public AccountLogic(IConnectionFactory connectionFactory, IAccountService accountService, ICustomerService customerService,ITransactionService transactionService ,ITimeProvider timeProvider) : base(connectionFactory)
        {
            // 確保帳戶服務不為空，否則拋出異常
            ...
            // 確保交易服務不為空，否則拋出異常
            _transactionService = transactionService ?? throw new ArgumentNullException(nameof(transactionService), "Transaction service cannot be null.");
        ...
        public async Task<bool> CloseAccount(CloseAccountRequest closeAccountRequest)
        {
            ...
            using (var dataAccess = await CreateConnectionAsync(transaction: true))
            {
                ...
                {
                    // 將數據訪問對象分配給帳戶服務
                    _accountService.DataAccess = dataAccess;

                    // 將數據訪問對象分配給交易服務
                    _transactionService.DataAccess = dataAccess;

                    // 獲得關閉帳戶的餘額
                    var balance = await _accountService.GetBalance(Convert.ToInt64(closeAccountRequest.AccountId));
                    ...
                    // 插入交易記錄並獲取交易ID
                    var transactionIdFrom = await _transactionService.InsertTransaction(transactionFrom);
                    ...
                    // 呼叫帳戶服務的方法來關閉帳戶
                    var result = await _accountService.CloseAccount(Convert.ToInt64(closeAccountRequest.AccountId),AccountStatus.Closed ,_timeProvider.Now());
                    ...
                    if (closeAccountRequest.TransactionType == TransactionType.Transfer)
                    {
                        ...
                        // 插入交易記錄並獲取交易ID
                        var transactionIdTo = await _transactionService.InsertTransaction(transactionTo);
        ...
        public async Task<List<AccountDto>> GetAccounts(int customerId)
        {
            ...
            using (var dataAccess = await CreateConnectionAsync())
            {
                ...
                // 將數據訪問對象分配給客戶服務
                var accounts = await _accountService.ListAccountsByCustomer(customerId);
        ...
        public async Task<decimal?> GetBalance(long accountId)
        {
            ...
            using (var dataAccess = await CreateConnectionAsync())
            {
                ...
                // 將數據訪問對象分配給帳戶服務
                accountService.DataAccess = dataAccess;
        ...
        public async Task<bool> ModifyAccount(ModifyAccountRequest accountRequest)
        {
			...
			using (var dataAccess = await CreateConnectionAsync(transaction: true))
            {
                try
                {
					...
					// 獲取交易的金額變動
					var amountDelta = await _transactionService.GetAmountDeltaByTransactionId(accountRequest.TransactionId);

					// 如果金額變動不為零，代表有一筆交易需要處理，則調整帳戶餘額
					if (amountDelta != 0)
                    {
						// 調整帳戶餘額
						var result = await _accountService.AdjustBalance(accountRequest.Account.AccountId, amountDelta, _timeProvider.Now());
					...
					// 獲取當前帳戶信息
					var currentAccount = await _accountService.GetAccountById(accountRequest.Account.AccountId);
					...
					if (accountRequest.Account.Currency.ToString() != currentAccount.Currency)
                    {
						...
						var res1 = await _accountService.UpdateCurrency(accountRequest.Account.AccountId, accountRequest.Account.Currency.ToString(), _timeProvider.Now());
                    ...
                    {
						// 更新帳戶的狀態
						var res2 = await _accountService.UpdateStatus(accountRequest.Account.AccountId, accountRequest.Account.Status, _timeProvider.Now());
                    ...
                    {
                        // 更新帳戶的類型
                        var res3 = await _accountService.UpdateAccountType(accountRequest.Account.AccountId, accountRequest.Account.AccountType, _timeProvider.Now());
		...
		public async Task<long> OpenAccount(OpenAccountRequest accountRequest)
        {
			...
			using (var dataAccess = await CreateConnectionAsync(transaction: true))
			{
				try
				{
					...
					var existingCustomer = await _customerService.GetCustomerByEmailOrPhone(accountRequest.Email, accountRequest.Phone);
					// 如果客戶不存在，拋出異常
					...
					var accountId = await _accountService.InsertNewAccount(accountModel);
		...
		public async Task<bool> SetCurrency(long accountId, CurrencyCode currency)
        {
            ...
            using (var dataAccess = await CreateConnectionAsync(transaction: true))
            {
                try
                {
                    ...
                    // 呼叫帳戶服務的方法來更新帳戶的貨幣類型
                    var result = await _accountService.UpdateCurrency(accountId, currencyString, updateDate);
